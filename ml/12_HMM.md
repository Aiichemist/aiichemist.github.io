...
我们知道，机器学习模型可以从频率派和贝叶斯派两个方向考虑，在频率派的方法中的核心是优化问题，而在贝叶斯派的方法中，核心是积分问题，也发展出来了一系列的积分方法如变分推断，MCMC 等。

概率图模型最基本的模型可以分为**有向图（贝叶斯网络）** 和**无向图（马尔可夫随机场）** 两个方面，例如 GMM。

在这些基本的模型上，如果样本之间存在关联，可以认为样本中附带了**时序信息**，从而样本之间不独立同分布的，这种模型就叫做**动态模型**，隐变量随着时间发生变化，于是观测变量也发生变化：

```mermaid
graph LR;
	z1((z<sub>1</sub>))-->z2((z<sub>2</sub>))-->z3((z<sub>3</sub>));
...
2.  Kalman 滤波，状态变量是**连续线性**的

3.  粒子滤波，状态变量是**连续非线性**的
...
上图表示了四个时刻的隐变量变化。用参数 $\lambda=(\pi,A,B)$ 来表示，其中：
- $\pi$ 是**起始概率分布**
...
$o_t$ 来表示**观测变量**，$O$ 为**观测序列**，$V=\{v_1,v_2,\cdots,v_M\}$ 表示**观测的值域**，

$i_t$ 表示**状态变量**，$I$ 为**状态序列**，$Q=\{q_1,q_2,\cdots,q_N\}$ 表示**状态变量的值域**。

定义 $A=(a_{ij}=p(i_{t+1}=q_j|i_t=q_i))$ 表示**状态转移矩阵**，

$B=(b_j(k)=p(o_t=v_k|i_t=q_j))$ 表示**发射矩阵**。
...
1.  **预测 Evaluation**：$p(O|\lambda)$，Forward-Backward 算法

2.  **学习 Learning**：$\lambda=\mathop{argmax}\limits_{\lambda}p(O|\lambda)$，EM 算法（Baum-Welch）

3.  **解码 Decoding**：$I=\mathop{argmax}\limits_{I}p(I|O,\lambda)$，Vierbi 算法
...
下面，记 $\textcolor{red}{\alpha_t(i)=p(o_1,o_2,\cdots,o_t,i_t=q_i|\lambda)}$，所以，$\alpha_T(i)=p(O,i_T=q_i|\lambda)$。我们看到：
$$
p(O|\lambda)=\sum\limits_{i=1}^Np(O,i_T=q_i|\lambda)=\sum\limits_{i=1}^N\alpha_T(i)
$$
对 $\alpha_{t+1}(j)$：
$$\begin{align}\alpha_{t+1}(j)
&=p(o_1,o_2,\cdots,o_{t+1},\underline{i_{t+1}=q_j}|\lambda)\nonumber\\
&=\sum\limits_{i=1}^Np(o_1,o_2,\cdots,o_{t+1},\underline{i_{t+1}=q_j},\underline{i_t=q_i}|\lambda)\nonumber\\
&=\sum\limits_{i=1}^Np(o_{t+1}|o_1,o_2,\cdots,o_t,\underline{i_{t+1}=q_j},\underline{i_t=q_i},\lambda)p(o_1,\cdots,o_t,\underline{i_{t+1}=q_j},\underline{i_t=q_i}|\lambda)
\end{align}$$
利用观测独立假设：$p(o_t|i_t,i_{t-1},\cdots,i_1,o_{t-1},\cdots,o_1)=p(o_t|i_t)$
...
\end{align}$$
上面利用了齐次 Markov 假设得到了一个递推公式，这个算法叫做**前向算法**。

还有一种算法叫做**后向算法**，定义 $\textcolor{red}{\beta_t(i)=p(o_{t+1},o_{t+1},\cdots，o_T|i_t=i,\lambda)}$：
$$
...
其中，$X$ 是观测变量 $O$，$Z$ 是隐变量序列 $I$，$\theta$ 是系统参数 $\lambda$。于是：
...
$$\begin{split}
Q(\lambda,\lambda^{(t)})
&=\sum\limits_I\log p(O,I|\lambda)p(O,I|\lambda^t)\\
&=\sum\limits_I\left[\log \pi_{i_1}+\sum\limits_{t=2}^T\log a_{i_{t-1},i_t}+\sum\limits_{t=1}^T\log b_{i_t}(o_t)\right]p(O,I|\lambda^t)
\end{split}$$
对 $\pi^{t+1}$：
$$\begin{align}
\pi^{t+1}
&=\mathop{argmax}_\pi Q(\lambda,\lambda^{(t)})\\
&=\mathop{argmax}_\pi\sum\limits_I[\log\pi_{i_1}p(O,I|\lambda^t)]\nonumber\\
&=\mathop{argmax}_\pi\sum\limits_I[\log \pi_{i_1}\cdot p(O,i_1,i_2,\cdots,i_T|\lambda^t)]
\end{align}$$
上面的式子中，对 $i_2,i_2,\cdots,i_T$ 求和可以将这些参数消掉：
$$\pi^{t+1}=\mathop{argmax}_\pi\sum\limits_{i=1}^N[\log \pi_{i_1}\cdot p(O,i_1=q_i|\lambda^{(t)})]$$
...
$$
我们需要找到一个序列，其概率最大，这个序列就是在参数空间中的一个路径，可以采用**动态规划**的思想。

定义到达 $i_t=q_i$ 概率最大的值：
$$\delta_{t}(j)=\max\limits_{i_1,\cdots,i_{t-1}}p(o_1,\cdots,o_t,i_1,\cdots,i_{t-1},\underline{i_t=q_i})$$
于是：
$$
...
HMM 是一种**动态模型**，是由**混合树形模型**和**时序**结合起来的一种模型（类似 GMM + Time）。

对于类似 HMM 的这种状态空间模型，普遍的除了**学习任务**（采用 EM ）外，还有**推断任务**，

推断任务包括：（假设隐状态为 $z_i$，观测状态为 $x_i$）

1.  解码 Decoding(**Viterbi算法**)：已知观测序列求解最有可能的隐变量序列
$$Z=\arg\max_{Z}p(Z|X)$$

2.  证据概率probability of evidence(**前向算法**、**后向算法**)：已知模型参数求解观测序列出现的概率
$$p(X|\theta)=p(x_1,x_2,\cdots,x_T|\theta)$$
3.  滤波Filterting(**前向算法**)：已知观测序列，求解**最后时刻**的隐状态——Online
$$p(z_t|x_1,\cdots,x_t)=\frac{p(x_{1:t},z_t)}{p(x_{1:t})}=C\alpha_t(z_t)$$
4.  平滑Smoothing(**前向后向算法**)：已知观测序列，求解**中间某一时刻**的隐状态——Offline
$$p(z_t|x_1,\cdots,x_T)=\frac{p(x_{1:T},z_t)}{p(x_{1:T})}=\frac{\alpha_t(z_t)p(x_{t+1:T}|x_{1:t},z_t)}{p(x_{1:T})}$$
    根据概率图的条件独立性，有：
$$p(z_t|x_{1:T})=\frac{\alpha_t(z_t)p(x_{t+1:T}|z_t)}{p(x_{1:T})}=C\alpha_t(z_t)\beta_t(z_t)$$
    这个算法叫做**前向后向算法**。
5.  预测：已知观测序列，求解未来的隐变量 $p(z_{t+1},z_{t+2}|x_1,\cdots,x_t)$ 或观测变量 $p(x_{t+1},x_{t+2}|x_1,\cdots,x_t)$
