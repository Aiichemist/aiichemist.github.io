...
高斯图模型（高斯网络）是一种随机变量为**连续**的有向或者无向图。

**有向版本**的高斯图是**高斯贝叶斯网络**，**无向版本**的叫**高斯马尔可夫网络**。
...
2.  我们看方差矩阵的逆（**精度矩阵**或信息矩阵）：$\Lambda=\Sigma^{-1}=(\lambda_{ij})_{p\times p}$，有定理：

>[!tip]   $x_i\perp x_j|(X-\{x_i,x_j\})\Leftrightarrow\lambda_{ij}=0$
