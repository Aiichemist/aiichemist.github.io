...
我们已经知道概率模型可以分为，**频率派的优化问题**和**贝叶斯派的积分问题**。

从贝叶斯角度来看推断，对于 $\widetilde{x}$ 这样的新样本，需要得到：
$$p(\widetilde{x}|X)=\int_\theta p(\widetilde{x},\theta|X)d\theta=\int_\theta p(\theta|X)p(\widetilde{x}|\theta,X)d\theta=\mathbb{E}_{\theta|X}[P(\widetilde{x}|\theta)]$$
如果新样本和数据集独立，那么推断就是**概率分布依参数后验分布的期望**。

我们看到，推断问题的中心是**参数后验分布的求解**，推断分为：
...
\end{split}$$
第二个式子可以写为**变分**和 KL 散度的和：
$$L(q)+KL(q||p)$$
由于这个式子是常数，于是寻找 $q(z)\approx p(z|x)$ 就相当于对 $L(q)$ 最大值。
$$
...
$$\begin{split}
\int_Zq(Z)\log q(Z)dZ
&=\int_Z\prod\limits_{i=1}^Mq_i(Z_i)\sum\limits_{i=1}^M\log q_i(Z_i)dZ\\
&=\int_Z\prod\limits_{i=1}^Mq_i(Z_i)\left[\log q_1(Z_1)+\log q_2(Z_2)+\cdots+\log q_M(Z_M)\right]dZ
\end{split}$$
展开求和项中第一项为：
$$\begin{split}
\int_Z\prod\limits_{i=1}^Mq_i(Z_i)\log q_1(Z_1)dZ
&=\int_{Z_1}q_1(Z_1)\log q_1(Z_1)dZ_1\cdot
\underbrace{\int_{Z_2} q_2(Z_2)dZ_2}_{1}\cdot
\underbrace{\int_{Z_3} q_3dZ_3}_{1}\cdots
\underbrace{\int_{Z_M}q_MdZ_M}_{1}\\
...
$$
又由于我们只关注 $q_j(Z_j)$，将其他项看做常数：
$$\sum\limits_{i=1}^M\int_{Z_i}q_i(Z_i)\log q_i(Z_i)dZ_i=\int_{Z_j}q_j(Z_j)\log q_j(Z_j)dZ_j+Const$$
两项相减，令 $\mathbb{E}_{\prod\limits_{i\ne j}q_i(Z_i)}[\log p(X,Z)]=\log \hat{p}(X,Z_j)$ 可以得到：
$$\int_{Z_j}q_j(Z_j)\cdot\log\dfrac{\hat{p}(X,Z_j)}{q_j(Z_j)}dZ_j
=-KL(q_j||\hat{p}(X,Z_j))\le0$$
于是当 $q_j(Z_j)=\hat{p}(X,Z_j)$ 才能得到最大值。

我们看到，对每一个 $q_j$，都是固定其余的 $q_i$，求这个值，于是可以使用**坐标上升**的方法进行迭代求解，上面的推导针对单个样本，但是对数据集也是适用的。
...
从 $Z$ 到 $X$ 的过程叫做**生成过程**或**译码**，反过来的过程叫**推断过程**或**编码**过程。
...
\hat{q}(Z)=\mathop{argmax}_{q(Z)}L(q)
$$
假定 $q(Z)=q_\phi(Z)$，是和 $\phi$ 这个参数相连的概率分布。

于是 $\mathop{argmax}\limits_{q(Z)}L(q)=\mathop{argmax}\limits_{\phi}L(\phi)$，其中 $L(\phi)=\mathbb{E}_{q_\phi}[\log p_\theta(x^i,z)-\log q_\phi(z)]$，这里 $x^i$ 表示第 $i$ 个样本。
$$
