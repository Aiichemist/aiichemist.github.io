...
# 索引排序
df.sort_index(level=['col1','col2'])
```
- `ascending=False`:是否设置为升序(默认升序)(可以为布尔序列)
...
```python
# 创建滑窗对象
roller = df.rolling(window = n)
```
- 对第 $i$ 个元素操作时，会往前取 $n-1$ 个元素，即第 $i-n+1\thicksim i$ 个元素
#### 类滑窗函数
```python
# 取向前第n个元素的值
df.shift(periods = n)
...
df.pct_change()
```
- `periods = n`:可以为负，表示反方向操作（默认为1）
### 扩张窗口expanding
又称累计窗口，每次操作时窗口大小为由序列开始处至当前位置
```python
df.expanding()
```
#### 类扩张窗口函数
