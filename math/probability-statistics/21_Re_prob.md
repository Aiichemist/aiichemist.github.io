...
>[!note] **定义：**一次试验中可能出现也可能补出现的结果称为**随机事件**，简称**事件**，并用大写字母$A,B,\cdots$来表示。

必然事件>[!note] **定义：**每次试验中一定发生的事件，记为$\Omega$。

不可能事件>[!note] **定义：**每次试验中一定不发生的事件，记为$\varnothing$。
...
>[!note] **定义：**样本空间满足：只有有限个样本点（基本事件）；每个样本点（基本事件）发生的可能性一样（等可能）。
...
>[!note] **定义：**样本空间$\Omega$是一个可度量的有界区域；每个样本点发生的可能性都是一样，即样本点落入$\Omega$的某一可度量的子区域$S$的可能性大小与$S$的几何度量成正比，而与$S$的位置与形状无关。
...
独立试验>[!note] **定义：**称试验$E_1,E_2,\cdots,E_n$为相互独立的，如果分别于各个试验相联系的任意$n$个事件之间相互独立。

独立重复试验>[!note] **定义：**独立表示与各试验相联系的事件之间相互独立，其中重复表示每个事件在各次试验中出现的概率不便。

伯努利试验>[!note] **定义：**只针对失败、成功两种对立结果的试验，将伯努利试验重复进行$n$次，就是**$n$重伯努利试验**。
