...
对于**齐次方程**，$x_1=\cdots=x_n=0$一定是其解，称为其**零解**，若有一组不全为零的解，则称为其**非零解**。其一定有零解，但是不一定有非零解。

对于**非齐次方程**，只有$b_1\cdots b_n$不全为零才是。

令：
- **系数矩阵** $A_{m\times n}=\left[\begin{array}{ccc}a_{11} & \cdots & a_{1n} \\\vdots \\a_{m1} & \cdots & a_{mn}\end{array}\right]$，
- **未知数矩阵** $x_{n\times 1}=\left[\begin{array}{c}  x_1 \\\vdots \\x_n\end{array}\right]$，
- **常数项矩阵** $b_{m\times 1}=\left[\begin{array}{c}b_1 \\\vdots \\b_m\end{array}\right]$，
- **增广矩阵** $B_{m\times(n+1)}=\left[\begin{array}{c:c}\begin{matrix}  a_{11} & \cdots & a_{1n}\\\vdots \\a_{m1} & \cdots & a_{mn}\end{matrix}&\begin{matrix}b_1\\\\b_n\end{matrix}\end{array}\right]$。

所以$AX=\left[\begin{array}{c}a_11x_1+\cdots+a_{1n}x_n \\\cdots \\a_{m1}x_1+\cdots+a_{mn}x_n\end{array}\right]$。

从而$AX=b$等价于$\begin{cases}a_{11}x_1+\cdots+a_{1n}x_n=b_1 \\\cdots \\a_{m1}x_1+\cdots+a_{mn}x_n=b_n\end{cases}$，当$b=O$就是齐次线性方程。
...
$$\begin{bmatrix}
    ...
    \vdots & \ddots & \vdots \\
    a_{m1} & \cdots & a_{ms}
\end{bmatrix}_{m\times s}\begin{bmatrix}
    b_{11} & \cdots & a_{1n} \\
    \vdots & \ddots & \vdots \\
    b_{s1} & \cdots & b_{sn}
...
1. 当所有的约束条件其中**存在线性相关**，即一部分约束条件可以由其他约束表示，则代表这部分约束条件是没用的，实际上的约束条件变少，从而情况等于$m<n$，结果是**无数解**。

2. 当所有的约束条件**不存在线性相关**，但是**一部分约束条件互相矛盾**，则约束条件下就无法解出解，从而结果是**无实数解**。

3. 当所有的约束条件**不存在线性相关**，且**相互之间不存在矛盾情况**，这时候才会解出一个实数解，从而结果是有**唯一实解**。
...
$$(a_1,a_2,\cdots,a_n)\begin{bmatrix}
    x_1 \\
    x_2 \\
    \vdots \\
    x_n
\end{bmatrix}=b\text{，}\begin{bmatrix}
    a_{11} \\
    a_{21} \\
    \vdots \\
    a_{m1}
...
>[!quote]+ **基础解系：** 假如$\xi_1,\xi_2,\cdots,\xi_{n-r}$满足：
>1. 是方程组$Ax=0$的解；
>
>2. 线性无关；
>
...
>
>则称$\xi_1,\xi_2,\cdots,\xi_{n-r}$为$Ax=0$的**基础解系**。

当$r(A)<n$时讨论基础解系。

>[!quote]+ **通解：** 设$\xi_1,\xi_2,\cdots,\xi_{n-r}$是$Ax=0$的基础解系，则$k_1\xi_1+k_2\xi_2+\cdots+k_{n-r}\xi_{n-r}$是方程组$Ax=0$的通解，$k_1,k_2,\cdots,k_{n-r}$为任意常数。
...
>[!example]+ **例题：** 求齐次线性方程组$\left\{\begin{array}{l}x_1+x_2-3x_4-x_5=0 \\x_1-x_2+2x_3-x_4=0 \\4x_1-2x_2+6x_3+3x_4-4x_5=0 \\2x_1+4x_2-2x_3+4x_4-7x_5=0\end{array}\right.$的通解。
>
>解：系数矩阵$A=\begin{bmatrix}1 & 1 & 0 & -3 & -1 \\1 & -1 & 3 & -1 & 0 \\4 & -2 & 6 & 3 & -4 \\2 & 4 & -2 & 4 & -7\end{bmatrix}$，然后对其行变换，得到：
>
>$=\begin{bmatrix}1 & 1 & 0 & -3 & -1 \\0 & -2 & 2 & 2 & 1 \\0 & 0 & 0 & 3 & -1 \\0 & 0 & 0 & 0 & 0\end{bmatrix}=\begin{bmatrix}1 & 0 & 1 & 0 & -\dfrac{7}{6}  \\0 & 1 & -1 & 0 & -\dfrac{5}{6} \\0 & 0 & 0 & 1 & -\dfrac{1}{3} \\0 & 0 & 0 & 0 & 0\end{bmatrix},r(A)=3$。
>
...
>
>$\left\{\begin{array}{l}1\cdot x_1+1\cdot x_2+0\cdot x_3-3\cdot x_4-1\cdot x_5=0 \\0\cdot x_1-2\cdot x_2+2\cdot x_3+2\cdot x_4+1\cdot x_5=0 \\0\cdot x_1+0\cdot x_2+0\cdot x_3+3\cdot x_4-1\cdot x_5=0\end{array}\right.$，分别代入：
>
>$\xi_1$：$\left\{\begin{array}{l}1\cdot x_1+1\cdot x_2+0\cdot1-3\cdot x_4-1\cdot0=0 \\0\cdot x_1-2\cdot x_2+2\cdot1+2\cdot x_4+1\cdot0=0 \\0\cdot x_1+0\cdot x_2+0\cdot1+3\cdot x_4-1\cdot0=0\end{array}\right.,\xi_1=(-1,1,1,0,0)^T$。
>
>$\xi_2$：$\left\{\begin{array}{l}1\cdot x_1+1\cdot x_2+0\cdot0-3\cdot x_4-1\cdot1=0 \\0\cdot x_1-2\cdot x_2+2\cdot0+2\cdot x_4+1\cdot1=0 \\0\cdot x_1+0\cdot x_2+0\cdot0+3\cdot x_4-1\cdot1=0\end{array}\right.,\xi_2=(7,5,0,2,6)^T$。
>
...
>[!example]+ **例题：** 求非齐次线性方程组$\left\{\begin{array}{l}x_1+5x_2-x_3-x_4=-1 \\x_1-2x_2+x_3+3x_4=3 \\3x_1+8x_2-x_3+x_4=1 \\x_1-9x_2+3x_3+7x_4=7\end{array}\right.$的通解。
>
>解：对方程组提取出增广矩阵并进行行变换：
>
>$\begin{bmatrix}\begin{matrix}1 & 5 & -1 & -1 \\1 & -2 & 1 & 3 \\3 & 8 & -1 & 1 \\1 & -9 & 3 & 7\end{matrix}&\begin{matrix}    -1 \\3 \\1 \\7\end{matrix}\end{bmatrix}=\begin{bmatrix}\begin{matrix}1 & 5 & -1 & -1 \\0 & -7 & 2 & 4 \\0 & 0 & 0 & 0 \\0 & 0 & 0 & 0\end{matrix}&\begin{matrix}-1 \\4 \\0 \\0\end{matrix}\end{bmatrix}$。
>
...
>[!example]+ **例题：** $r(A_{4\times4})=2,\eta_1,\eta_2,\eta_3$为$Ax=b$的三个解向量，其中具有如下关系：
>
>$\left\{\begin{array}{l}\eta_1-\eta_2=(-1,0,3,-4)^T \\\eta_1+\eta_2=(3,2,1,-2)^T \\\eta_3+2\eta_2=(5,1,0,3)^T\end{array}\right.$，求$Ax=b$的通解。
>
...
>[!example]+ **例题：** 设$\xi_1,\xi_2,\xi_3$是方程组$Ax=0$的基础解系，则下列向量组也是方程组$Ax=0$的基础解系的是()。
>
>$\begin{split}&A.\xi_1-\xi_2,\xi_2-\xi_3,\xi_3-\xi_1 &B.\xi_1+\xi_2,\xi_2-\xi_3,\xi_3+\xi_1\\&C.\xi_1+\xi_2-\xi_3,\xi_1+2\xi_2+\xi_3,2\xi_1+3\xi_2 \quad\quad &D.\xi_1+\xi_2,\xi_2+\xi_3,\xi_3+\xi_1\end{split}$
>
>---
>
>解：需要判断基础解系是否线性无关，需要对应的行列式值非0。
>
>对于$D$：$(\xi_1+\xi_2,\xi_2+\xi_3,\xi_3+\xi_1)=(\xi_1,\xi_2,\xi_3)\begin{bmatrix}1 & 0 & 1 \\1 & 1 & 0 \\0 & 1 & 1\end{bmatrix}\neq0$，所以$D$线性无关，从而为基础解系。

>[!example]+ **例题：** 设$\xi_1=[1,-2,3,1]^T,\xi_2=[2,0,5,-2]^T$是齐次线性方程组$A_{3\times4}x=0$的解，且$r(A)=2$，则下列向量中是其解向量的是()。
>
>$$\begin{split}&A.\alpha_1=[1,-2,3,2]^T \quad\quad&B.\alpha_2=[0,0.5,-2]^T\\ &C.\alpha_3=[-1,-6,-1,7]^T&D.\alpha_4=[1,6,1,6]^T\end{split}$$
>
...
>[!example]+ **例题：** 已知$A=[\alpha_1,\alpha_2,\alpha_3,\alpha_4]$，其中$\alpha_1,\alpha_2,\alpha_3,\alpha_4$是四维列向量，且$\alpha_1=2\alpha_2+\alpha_3,r(A)=3$，若$\beta=\alpha_1+2\alpha_2+3\alpha_3+4\alpha_4$，求线性方程组$Ax=\beta$的通解。
>
...
>[!example]+ **例题：** 已知线性方程组$A=\left\{\begin{array}{l}x_1+x_2=0 \\x_2-x_4=0\end{array}\right.,B=\left\{\begin{array}{l}x_1-x_2+x_3=0 \\x_2-x_3+x_4=0\end{array}\right.$，求方程组的公共解。
>
>解：$A=\begin{bmatrix}1 & 1 & 0 & 0 \\0 & 1 & 0 & -1\end{bmatrix},B=\begin{bmatrix}1 & -1 & 1 & 0 \\0 & 1 & -1 & 1\end{bmatrix}$。
>
...
要求$A$和$B$的非零公共解，即求联立矩阵$\begin{bmatrix}A \\B\end{bmatrix}x=0$的非零解。对这个矩阵求出基础解系。
...
>[!example]+ **例题：** 线性方程组$A=\left\{\begin{array}{l}x_1+3x_3+5x_4=0 \\x_1-x_2-2x_3+2x_4=0 \\2x_1-x_2+x_3+3x_4=0\end{array}\right.$，在其基础上加一个方程$B=\left\{\begin{array}{l}x_1+3x_3+5x_4=0 \\x_1-x_2-2x_3+2x_4=0 \\2x_1-x_2+x_3+3x_4=0 \\4x_4+ax_2+bx_3+13x_4=0\end{array}\right.,ab$满足什么条件，$AB$是同解方程组。
>
>解：$B$在$A$的基础上增加一个方程，即多增加了约束，从而$B$的解一定为$A$的解的子集。所以只要$A$的解也满足$B$的解就是同解方程组。
>
>$A=\begin{bmatrix}1 & 0 & 3 & 5 \\0 & -1 & -5 & -3 \\0 & 0 & 0 & -4\end{bmatrix},s=n-r=4-3=1,\xi=(-3,-5,1,0)^T,k\xi=k(-3,-5,1,0)^T=(-3k,-5k,k,0)^T$。
>
>所以这个对于$B$而言必然满足前三行，若要整体满足，就也要满足$B$的第四行，所以直接代入第四行：$4(-3k)+a(-5k)+bk+0=k(-12-5a+b)=0$。
>
>又$k$为任意数，所以$-12-5a+b=0$，即$b=5a+12$。

>[!example]+ **例题：** 设$A$为$n$阶实矩阵，$A^T$是$A$的转置矩阵，证明方程组$\Lambda:Ax=0$和$\Upsilon:A^TAx=0$是同解方程组。
>
...
>[!success]+ **定理：** $r(A)=r(A^T)=r(A^TA)=r(AA^T)$。
