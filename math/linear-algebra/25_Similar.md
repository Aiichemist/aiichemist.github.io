...
-   $A^{-1}\xi=\dfrac{1}{\lambda}\xi$。

-   $A^*\alpha=\dfrac{\vert A\vert}{\lambda}\xi$。

### 特征向量性质

-   $k$重特征值$\lambda$至多只有$k$个线性无关的特征向量。一共有$k$个特征向量。
...
>[!success]+ **定理：** 若矩阵$A$为对角线矩阵，则特征值为对角线上元素。

>[!success]+ **定理：** 若$n$阶矩阵$A$行或列对应成比例，即$r(A)=1$，则$\lambda_1=\lambda_2=\cdots=\lambda_{n-1}=0,\lambda_n=tr(A)$。

>[!warning] **注意：** 特征向量因为要求不为0，所以需要$k\neq0$。

>[!warning] **注意：** 得到多重特征值时要全部写出，$\lambda_1=\lambda_2=\cdots=\lambda_n=\lambda$。

>[!example]+ **例题：** 求$A=\begin{bmatrix}
    2 & -2 & 0 \\
    -2 & 1 & -2 \\
    0 & -2 & 0
...
>[!example]+ **例题：** 设$A$为$n$阶矩阵，且$A^T=A$（此时$A$就是幂等矩阵）。
...
>[!quote]+ **定义：** 设$A,B$是两个$n$阶方阵，若存在$n$阶可逆矩阵$P$，使得$P^{-1}AP=B$，则称$A$相似于$B$，记为$A\sim B$。
...
>[!quote]+ **定义：** 设$n$阶矩阵$A$，若存在$n$阶可逆矩阵$P$，使得$P^{-1}AP=\Lambda$，其中$\Lambda$为对角矩阵（纯量阵，即对角线元素不全为0，其他元素全为0），则称$A$可**相似对角化**，记为$A\sim\Lambda$，称$\Lambda$是$A$的**相似标准形**。
...
$A(\xi_1,\xi_2,\cdots,\xi_n)=(\xi_1,\xi_2,\cdots,\xi_n)\begin{bmatrix}
    \lambda_1 \\
     & \lambda_2 \\
     & & \ddots \\
     & & & \lambda_n
...
\end{bmatrix}=\begin{bmatrix}
    ...
    0 & 0 & 0 \\
    0 & 0 & 0
\end{bmatrix},s=n-r=2$，所以有两个基础解系向量。
...
>[!example]+ **例题：** 设$A=\begin{bmatrix}
    ...
    2 & 5 & -4 \\
    -2 & -4 & 5
\end{bmatrix}$，求可逆矩阵$P$，使得$P^{-1}AP=\Lambda$。
...
令$P=(\xi_1,\xi_2,\xi_3)=\begin{bmatrix}
    -2 & 2 & 1 \\
    1 & 0 & 2 \\
    0 & 1 & -2
...
>[!quote]+ **定义：** 若$\alpha=(a_1,a_2,\cdots,a_n),\beta=(b_1,b_2,\cdots,b_n)$，则内积$(\alpha,\beta)=a_1b_1+a_2b_2+\cdots+a_nb_n$。
...
>[!quote]+ **定义：** $A^T=A$则$A$就是对称矩阵，若$A$的元素都是实数，则$A$是实对称矩阵。
...
>[!example]+ **例题：** 设$A=\begin{bmatrix}
    ...
    2 & 5 & -4 \\
    -2 & -4 & 5
\end{bmatrix}$，求正交矩阵$Q$，使得$Q^{-1}AQ=\Lambda$。
